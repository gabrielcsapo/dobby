...
from .widgets.icon import Icon, DOBBY_ICON
...
class App(object):
    ...
    def __init__(self, name, app_id, icon=None, startup=None):
        ...
        Icon.app_icon = Icon.load(icon, default=DOBBY_ICON)
    ...
    def hide(self):
        app = NSApplication.sharedApplication()
        return app.hide_(app)

    def toggle(self):
        app = NSApplication.sharedApplication()
        hidden = cftype_to_value(app.valueForKey_(get_NSString('hidden')))
        if(hidden):
            self.show()
        else:
            self.hide()
